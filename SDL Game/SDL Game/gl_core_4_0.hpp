...
namespace gl
{
	...
	enum
	{
		...
		//UNIFORM_SIZE taken from ext: ARB_uniform_buffer_object
		//UNIFORM_NAME_LENGTH taken from ext: ARB_uniform_buffer_object
		...
		//MAX_VERTEX_STREAMS taken from ext: ARB_gpu_shader5
	...
	inline GLvoid* MapBuffer(GLenum target, GLenum access){return _detail::MapBuffer(target, access);}
	...
	inline void GetAttachedShaders(GLuint program, GLsizei maxCount, GLsizei *count, GLuint *obj){_detail::GetAttachedShaders(program, maxCount, count, obj);}
	...
	inline GLvoid* MapBufferRange(GLenum target, GLintptr offset, GLsizeiptr length, GLbitfield access){return _detail::MapBufferRange(target, offset, length, access);}
